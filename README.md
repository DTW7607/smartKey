...
内置 3.5mm 智键的 macOS 菜单栏程序：为单击、双击和长按绑定键盘、多媒体或脚本动作，触发时用玻璃气泡显示动作名称，按下时右下角出现按压缩罩。需要 macOS 26。
...
- 每个动作名称为 1–8 个可见字符。键盘/媒体名称可修改，脚本名称同步用于绑定及气泡。气泡图标区分运行、已发送、完成或失败；实际结果可在设置中查看。
...
`swift test` 覆盖设备选择与输出守卫、手势会话、绑定/配置恢复、脚本包、外部保存、脚本超时/取消/大输出/进程组清理，以及设置页面渲染。默认测试使用模拟设备与临时脚本，不修改真实音频路由或独占硬件；渲染结果输出到 `/tmp/smartKey-*.png`。实际插拔、媒体路由、系统授权、锁屏/恢复和按键仍需设备验收，详见 [首版验证记录](docs/v1-validation.md)。
