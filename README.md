...
3. 按提示选择声音输出。只有一个可用的非耳机孔输出时会自动选择；有多个时可手动确认，倒计时结束后使用列表中的第一个可用输出，时长与设备类型选择相同。设置窗口可见时，新流程直接在「通用 → 设备」内完成，并使用相同倒计时；关闭或最小化设置会立即执行超时选择，已有弹窗流程不受设置窗口开关影响。音频切换及 HID 连接成功后，才开始接收按键。
4. 每次启动若缺少辅助功能权限，应用会显示授权引导；未授权时不创建菜单栏，关闭最后一个窗口即退出。打开设置期间仍正常更新设备连接和音频列表，可使用「通用」设置，也可查看灰色的动作配置和脚本管理页面，但无法操作或执行动作。点击「去授权…」后，通过系统确认弹窗打开系统设置并允许智键。输入监控仅在线控访问被权限阻止时提示申请，设置页只在它已授权时显示。授权后打开「设置 → 动作配置」，为单击、双击或长按选择动作并保存绑定。
