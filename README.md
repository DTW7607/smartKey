...
- 启动检查已插入的内置 3.5mm 设备，同时监听后续插拔。检测到插入时，右下角先播放黑色遮罩动画，再延迟显示设备类型选择气泡；时长见 `popup.conf` 的 `insertionMaskDurationMs` / `insertionPopupDelayMs`。
...
- 菜单栏仅保留状态、「重新配置设备…」和「退出」。手动重新配置不启动自动选择倒计时。
...
- 手势、插入动画、选择超时和配置窗口尺寸见 `popup.conf`。用户上次选择的设备类型不写入 conf，由程序单独持久化。
...
HID 回归测试（先退出正在运行的 smartKey；会短暂独占内置 Audio 线控三次，不切换音频路由）：
